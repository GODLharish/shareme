...
// Upload files for logged-in users
const uploadFiles = async (req, res) => {
  ...
  try {
    ...
    for (const file of req.files) {
      ...
      const filename = `${Date.now()}-${shortid.generate()}-${file.originalname}`;
      ...
      const fileObj = {
        ...
        createdBy: userId,
        gridfsId: uploadStream.id // Store GridFS file ID
      };
...
const downloadInfo = async (req, res) => {
  ...
  try {
    ...
    const downloadUrl = `${process.env.BASE_URL}/api/files/download/${file._id}`;
...
const guestDownloadInfo = async (req, res) => {
  ...
  try {
    ...
    const downloadUrl = `${process.env.BASE_URL}/api/files/download/guest/${file._id}`;
...
const downloadFile = async (req, res) => {
  ...
  try {
    ...
    const bucket = getGridFSBucket();
    const downloadStream = bucket.openDownloadStreamByName(file.path);
...
const sendLinkEmail = async (req, res) => {
  ...
  try {
    ...
    const mailOptions = {
      from: `"ShareMe File Sharing" <${process.env.MAIL_USER}>`,
      to: email,
      ...
      html: `
        <div style="font-family: Arial, sans-serif; line-height: 1.6;">
          ...
          <p>Hello,</p>
          <p>You have been sent a file using <strong>ShareMe</strong>.</p>
          <p><strong>File Name:</strong> ${file.name}</p>
          <p><strong>File Type:</strong> ${file.type}</p>
          <p><strong>Size:</strong> ${(file.size / 1024).toFixed(2)} KB</p>
          <p><strong>Download Link:</strong></p>
          <p><a href="${process.env.BASE_URL}/f/${file.shortUrl.split('/').pop()}" target="_blank" style="color: #3366cc;">Click here to download your file</a></p>
          ${file.expiresAt
          ...
          <p>Thank you for using ShareMe!</p>
        </div>
      `
    };
...
const generateQR = async (req, res) => {
  ...
  try {
    ...
    const fileUrl = `${process.env.BASE_URL}/f/${file.shortUrl.split('/').pop()}`;
...
const resolveShareLink = async (req, res) => {
  ...
  try {
    const file = await File.findOne({ shortUrl: `/f/${code}` });
    ...
    return res.status(200).json({
      ...
      size: file.size,
      type: file.type || "file",
      previewUrl: `${process.env.BASE_URL}/api/files/download/${file._id}`,
      isPasswordProtected: file.isPasswordProtected || false,
      expiresAt: file.expiresAt || null,
      status: file.status || "active",
    });
